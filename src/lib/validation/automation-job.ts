...
export const automationJobSnapshotV2Schema = z.object({
  ...
  buildingWarrant: z.object({
    ...
    feeCalculation: z.discriminatedUnion('calculationStatus', [
      z.object({
        calculationStatus: z.literal('CALCULATED'),
        scheduleVersion: z.string().min(1),
        effectiveFrom: z.string().date(),
        submissionType: z.literal('BUILDING_WARRANT'),
        valueOfWorksMinorUnits: z.number().int().nonnegative().nullable(),
        calculationBasisMinorUnits: z.number().int().nonnegative().nullable(),
        baseFeeMinorUnits: z.number().int().nonnegative(),
        multiplierPercent: z.number().int().positive(),
        subtotalMinorUnits: z.number().int().nonnegative(),
        adjustments: z.array(z.object({
          type: z.enum(['CERTIFIER_OF_DESIGN', 'CERTIFIER_OF_CONSTRUCTION']),
          label: z.string().min(1),
          amountMinorUnits: z.number().int().nonpositive(),
        })),
        totalFeeMinorUnits: z.number().int().nonnegative(),
        specialRule: nullableText,
        explanation: z.array(z.string().min(1)),
      }),
      z.object({
        calculationStatus: z.literal('NEEDS_INFORMATION'),
        scheduleVersion: z.string().min(1),
        effectiveFrom: z.string().date(),
        submissionType: z.literal('BUILDING_WARRANT'),
