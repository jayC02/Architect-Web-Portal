...
export const desktopJobStatusSchema = z.object({
  ...
  result: z.object({
    ...
    failureCategory: z.string().trim().max(80).nullable().optional(),
    errorHeadline: z.string().trim().max(200).nullable().optional(),
    stageDescription: z.string().trim().max(200).nullable().optional(),
    progressionMessage: z.string().trim().max(500).nullable().optional(),
    ...
    recoveryFields: z.array(z.string().trim().min(1).max(80)).max(20).optional(),
