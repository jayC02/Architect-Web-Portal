...
export function AgentDefaultsForm({
  ...
  return <form data-api-form data-field-errors data-action="/api/settings/organisation-defaults" data-method="PUT" data-success-message="Practice settings saved." className={compact ? 'grid gap-4' : 'panel grid gap-7 rounded-lg p-5 sm:p-6'}>
    <section aria-labelledby="practice-details-heading">
      {!compact && <div><h2 id="practice-details-heading" className="text-xl font-semibold">Practice details</h2><p className="mt-1 text-sm leading-6 text-stone-500">The practice identity used across Architect Pro applications.</p></div>}
      <label className={`${compact ? '' : 'mt-5'} block`}><span className="label">Practice name</span><input required name="practiceName" defaultValue={defaults?.practiceName ?? organisationName} className="field" disabled={!canManage} /></label>
    </section>
    <section aria-labelledby="application-contact-heading" className={compact ? '' : 'border-t border-stone-200 pt-7'}>
      {!compact && <div><h2 id="application-contact-heading" className="text-xl font-semibold">Default application contact</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-stone-500">Used as the default contact when preparing new applications. Project or application-specific details can still override these values.</p></div>}
      <div className={`${compact ? '' : 'mt-5'} grid gap-4 sm:grid-cols-2`}><label className="block"><span className="label">First name</span><input name="agentFirstName" defaultValue={defaults?.agentFirstName ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Last name</span><input name="agentLastName" defaultValue={defaults?.agentLastName ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Email</span><input type="email" name="agentEmail" defaultValue={defaults?.agentEmail ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Phone</span><input name="agentPhone" defaultValue={defaults?.agentPhone ?? ''} className="field" disabled={!canManage} /></label></div>
      {!compact && <div className="mt-6"><h3 className="text-sm font-semibold text-ink">Contact postal address</h3><p className="mt-1 text-sm text-stone-500">Used only where the application contact requires a postal address.</p><div className="mt-4 grid gap-4 sm:grid-cols-[minmax(120px,0.35fr)_minmax(0,1fr)]"><label className="block"><span className="label">Building number</span><input name="agentBuildingNumber" defaultValue={defaults?.agentBuildingNumber ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Address line 1</span><input name="agentAddressLine1" defaultValue={defaults?.agentAddressLine1 ?? ''} className="field" disabled={!canManage} /></label></div><label className="mt-4 block"><span className="label">Address line 2</span><input name="agentAddressLine2" defaultValue={defaults?.agentAddressLine2 ?? ''} className="field" disabled={!canManage} /></label><div className="mt-4 grid gap-4 sm:grid-cols-3"><label className="block"><span className="label">Town / city</span><input name="agentTownCity" defaultValue={defaults?.agentTownCity ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Postcode</span><input name="agentPostcode" defaultValue={defaults?.agentPostcode ?? ''} className="field" disabled={!canManage} /></label><label className="block"><span className="label">Country</span><input name="agentCountry" defaultValue={defaults?.agentCountry ?? 'United Kingdom'} className="field" disabled={!canManage} /></label></div></div>}
    </section>
